...
# Default-Stop:      0 1 6
# Short-Description: Lance rTorrent dans un screen
# Description:       Ce service s'assure que rTorrent sera relancé 
#                    en cas de reboot.
### END INIT INFO
...
do_start()
{
  # Return
	...
	start-stop-daemon --start --quiet --pidfile $PIDFILE --exec su -c \“/usr/bin/screen -fn -dmS rtd nice -19 $NAME\” $USER --test > /dev/null \
		|| return 1
	start-stop-daemon --start --quiet --pidfile $PIDFILE --exec su -c \“/usr/bin/screen -fn -dmS rtd nice -19 $NAME\” $USER -- \
		$DAEMON_ARGS \
		|| return 2
}
